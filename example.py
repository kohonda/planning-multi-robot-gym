...
def planning_policy(observation, info, forward_weight=12, obstacle_weight=6666):
    def calculate_closest_obstacle_distance(robot_position, barriers_positions):
        return (
            np.sum(np.sqrt((barriers_positions - robot_position) ** 2), axis=1).min()
            - info["barrier_radius"]
            - info["robot_radius"]
        )
    ...
    for r in range(info["n_robots"]):
        ...
        vL_possible_array = (
            vL - info["max_acceleration"] * info["dt"],
            vL,
            vL + info["max_acceleration"] * info["dt"],
        )
        vR_possible_array = (
            vR - info["max_acceleration"] * info["dt"],
            vR,
            vR + info["max_acceleration"] * info["dt"],
        )
        ...
        for vL_possible in vL_possible_array:
            for vR_possible in vR_possible_array:
                if (
                    info["max_velocity"] >= vL_possible >= -info["max_velocity"]
                    and info["max_velocity"] >= vR_possible >= -info["max_velocity"]
                ):
                    (x_predict, y_predict, theta_predict, _) = info["predict_position"](
                        vL_possible, vR_possible, x, y, theta, info["tau"]
                    )
                    ...
                    if distance_to_obstacle < info["robot_radius"]:
                        obstacle_cost = obstacle_weight * (
                            info["robot_radius"] - distance_to_obstacle
                        )
                    else:
                        obstacle_cost = 0.0
        ...
        barrier_positions = np.vstack(
            (barrier_positions, np.array([x_chosen, y_chosen]))
        )
...
if __name__ == "__main__":
    env = planning_multi_robot_gym.make(
        "PlanningMultiRobot-v0", render_mode="human", n_robots=5, n_barriers=80
    )
    ...
    for _ in range(1000):
        observation, reward, terminated, truncated, info = env.step(
            planning_policy(observation, info)
        )
        env.render()
